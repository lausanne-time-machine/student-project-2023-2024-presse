...
async function deploy({ config, message, force, deployPollInterval = DEPLOY_POLL_INTERVAL_MS, maxConcurrency }, effects = defaultEffects) {
  ...
  clack2.intro(`${inverse(" observable deploy ")} ${faint(`v${"1.7.0"}`)}`);
  ...
  if (!doBuild && !force && effects.isTty) {
    ...
    if (mostRecentSourceMtimeMs > leastRecentBuildMtimeMs) {
      ...
      clack2.log.info(wrapAnsi(`You built this project ${formatAge(buildAge)}.`, effects.outputColumns));
    }
    const choice = await clack2.confirm({
  ...
  const progressSpinner = clack2.spinner();
